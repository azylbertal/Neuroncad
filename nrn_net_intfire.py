...
class Neuron(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.mod=neuron.h.IntFire1(.5)
        self.stm=neuron.h.NetStim(.5)
        self.stm.number=0
        self.con=neuron.h.NetCon(self.stm, self.mod)
        self.con.weight[0]=1.1
        self.mem=0
...
class Axon():
    def __init__(self, startp, endp, points):
        ...
        self.con=neuron.h.NetCon(startp.mod, endp.mod, 1.0, self.len*step, self.w)
...
def run_loop(all_neurons):
    ...
    while running:
        ...
        for counter, neur in enumerate(all_neurons.sprites()):
            ...
            new_mem=neur.mod.M()
            if neur.mem<=1 and new_mem==2:
                for ax in neur.axons:
                    APs.append(AP(ax))

            neur.mem=new_mem
            if neur.mem==2 and neur.fire_counter==0:
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            ...
            for counter, neur in enumerate(all_neurons.sprites()):
                if neur.rect.collidepoint([x, y]):
                    ...
                    neur.con.event(neuron.h.t+neuron.h.dt)   
                        #neur.stm.start=neuron.h.t+neuron.h.dt
#                        con=neuron.h.NetCon(stm, neur.mod)
#                        con.weight[0]=1.0
#                        print con.weight[0]
...
#        t+=step         
        ...
        v=np.append(v[1::], [all_neurons.sprites()[0].mod.M()])
        vmax=np.max(v)
        vmin=np.min(v)-1
