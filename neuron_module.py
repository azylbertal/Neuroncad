...
import math
import neuron
from camera_module import receptiveField
...
class Axon(object):
    def __init__(self, startp, endp, points, tp, weight, start_id, end_id, time_step, interp=True):
        ...
        if tp=='excitatory' or tp=='irsensor' or tp=='visual' or tp=='auditory':
            ...
            self.cl=RED
            self.syn.e=-90.0
        self.con=neuron.h.NetCon(startp.mod(0.5)._ref_v, self.syn, 25.0, self.len*time_step, self.w, sec=startp.mod)
...
class AP(object):
    ...
    def draw_and_advance(self, to_draw):
        self.pos+=1
        if to_draw:
		oldc=pygame.draw.circle(self.screen, WHITE, map(int, self.axon.points[self.old_pos]), 7)
